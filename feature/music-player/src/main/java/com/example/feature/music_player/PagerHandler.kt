...
fun PagerHandler(
    ...
    onMoveToIndex: (Int, String) -> Unit,
) {
    // when current music track is end and moved to the next music it causes mediaId change and this execute
    LaunchedEffect(key1 = currentPlayerMediaId) {
        val index = pagerMusicList.indexOfFirst { it.musicId.toLong() == currentPlayerMediaId }
        if (currentPagerPage == index && pagerState.isScrollInProgress && index != -1) return@LaunchedEffect
    ...
    LaunchedEffect(key1 = pagerState.settledPage, key2 = pagerState.isScrollInProgress) {
        ...
        if (pagerState.settledPage != currentPagerPage && !pagerState.isScrollInProgress) {
            onMoveToIndex(
                ...
                pagerMusicList[pagerState.settledPage].musicId,
            )
